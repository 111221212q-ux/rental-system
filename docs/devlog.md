...
## 2026-05-28 — 修复管理员函数损坏 + 仪表盘/后台提醒优化

### 修复
- **管理员函数损坏修复**：node 脚本修改导致 3 个函数损坏
  - 删除 `approveRental` 重复函数头（第 1001 行多出一行声明）
  - `renderAdminPickup` 补回 `const p/t/e` 变量声明（变量丢失导致 `p is not defined`）
  - `renderAdminActive` 补回函数头（函数体孤立，管理员无法查看"租借中"物品）
- **待领取 Badge 一直是0**：`loadData` 和 `renderAdminApprovals` 新增 `pickup-badge` 更新逻辑（原来只在点击"待领取"tab 时更新）

### 变更
- **仪表盘提醒精简**：只显示逾期提醒，不再显示即将到期（daysLeft）提醒
- **严重逾期独立档次**：逾期超过 15 天的租借在仪表盘用深红色高亮；管理后台新增"严重逾期(>15天)"统计卡片，可点击查看详情
- **仪表盘提醒可收起**：逾期提醒栏左上角有"收起/展开"按钮，点击折叠提醒列表
...
- **管理员搜索功能**：5个管理标签页（待审核/待领取/租借中/物品管理/用户管理）加入搜索框，输入即搜，按用户名/物品名/邮箱实时筛选
- **权限修复**：测试数据生成/清理端点改为 `superadmin` 权限，前端按钮仅超级管理员可见
- **使用指南升级**：使用说明弹窗改为双标签页（用户指南 + 管理员指南），涵盖注册到管理的全流程说明
...
- **内联错误提示**：注册表单新增 `#reg-error` 错误信息区域（类似登录表单），密码不够长/邮箱为空等验证失败时显示内联错误提示，不会一闪而过
- **超级管理员删除用户**：用户管理操作列新增"删除"按钮，确认后删除用户及其所有租借记录和评论（不可恢复）
